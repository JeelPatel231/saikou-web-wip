...
<div class="container">
    <div class="gridtable padded-32">
        ...
        <span v-if="details.episodes">Total Episodes</span> <span v-if="details.episodes">{{details.episodes}}</span>
        <span v-if="details.chapters">Total Chapters</span> <span v-if="details.chapters">{{details.chapters}}</span>
        <span v-if="details.duration">Average Duration</span> <span v-if="details.duration">{{details.duration}}</span>
        <span>Format</span> <span>{{details.format}}</span>
        <span>Source</span> <span>{{details.source}}</span>
        <span v-if="details.studios.nodes.length">Studio</span> <span v-if="details.studios.nodes.length">{{details.studios.nodes[0].name}}</span>
        <span v-if="details.season">Season</span> <span v-if="details.season">{{details.season}}</span>
        <span>Start Date</span> <span>{{`${details.startDate.year}-${details.startDate.month}-${details.startDate.day}`}}</span>
        <span>End Date</span> <span>{{`${details.endDate.year}-${details.endDate.month}-${details.endDate.day}`}}</span>
    </div>
